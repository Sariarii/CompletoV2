...
const COLORS = ['#1B4FE0', '#FF7849', '#10B981', '#A78BFA', '#F59E0B', '#EC4899', '#06B6D4', '#84CC16'];

function formatEUR(n: number): string {
  return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(n);
}

export default function ServiceBreakdownChart({ data }: { data: ServiceData[] }) {
  const chartData = data.map((d) => ({ name: d.service, value: d.grossEUR }));
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, minWidth: 140 }}>
        {chartData.map((d, i) => (
          <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: '#6B7693', flex: 1 }}>{d.name.replace(/^Amazon /, '').replace(/^AWS /, '')}</span>
